...
print("------------------ Pairwise Bandwidth -----------------")
print()

# Measure bandwidth between host pairs using iperf
for i, host1 in enumerate(hosts):
    for j, host2 in enumerate(hosts):
        if i != j:
            bandwidth_result = net.iperf((host1, host2))
            print(f"{host1.name} -> {host2.name}: {bandwidth_result[0]}")
net.stop()
